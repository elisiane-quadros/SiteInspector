...
from backend.utils.html_fetcher import browser_session, close_browser, init_browser
...
async def lifespan(app: FastAPI):
    """
    Inicializa o navegador Chromium na inicialização do servidor
    e o finaliza no desligamento.
    """
    await init_browser()
    logger.info("Navegador Chromium inicializado (singleton). Servidor operacional.")
    yield
    logger.info("Encerrando servidor — fechando navegador...")
    await close_browser()
...
async def check_accessibility(data: AnalysisRequest):
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Erro interno ao processar a inspeção. Tente novamente em instantes.",
        )


@app.get("/")
async def root():
    return {"status": "online", "app": "A11y Inspector API", "version": "2.0.0", "docs": "/docs"}
